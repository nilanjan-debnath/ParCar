...
class parkDetails(ctk.CTkScrollableFrame):
    def __init__(self, master, **kwargs):
        ...
        for i in data["Parking_Spots"]:
            ...
            def change_state(n):
                park_states[n] = (park_states[n] + 1) % len(states)
                text, color = states[park_states[n]]
...
class MyTabView(ctk.CTkTabview):
    def __init__(self, master, **kwargs):
        ...
        # Add widgets to tabs
        self.label_dashboard = ctk.CTkLabel(master=self.tab("Dashboard"), text="This is Tab 1").grid(row=0, column=0, padx=20, pady=10)

        self.my_frame = parkDetails(master=self.tab("Dashboard"), width=1220, height=550)
        self.my_frame.grid(row=1, column=0, columnspan=20)
        
               

        # Camera setup
        self.cap = cv2.VideoCapture("carPark.mp4")  # Adjust camera index if needed
    ...
    def checking_parking_space(self, imgPro):
        ...
        for i, pos in enumerate(posList):
            ...
            #     # threading.Thread(target=localUpdate(slot_number)).start()
